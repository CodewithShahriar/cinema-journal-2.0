  ...
  return (
    ...
    >
      <div className="relative h-32 w-[86px] shrink-0 overflow-hidden rounded-xl bg-muted">
        {movie.poster ? (
          <img src={movie.poster} alt={movie.title} className="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" loading="lazy" />
        ) : (
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-1 text-xs text-muted-foreground">
          {movie.year && (
            <span className="inline-flex items-center gap-1"><Calendar className="h-3 w-3" />{movie.year}</span>
          )}
          {movie.runtime && (
            <span className="inline-flex items-center gap-1"><Clock className="h-3 w-3" />{movie.runtime}m</span>
          )}
          {movie.status === "watched" && movie.rating != null && (
            <span className="inline-flex items-center gap-1 text-gold"><Star className="h-3 w-3 fill-current" />{movie.rating.toFixed(1)}</span>
          )}
        </div>
        <div className="mt-2 flex flex-wrap gap-1">
          {movie.genres.slice(0, 3).map((g) => (
            <span key={g} className="rounded-full bg-secondary px-2 py-0.5 text-[10px] font-medium text-secondary-foreground">
              {g}
            </span>
          ))}
        </div>
        {movie.note && (
          <p className="mt-auto line-clamp-2 pt-2 text-xs italic text-muted-foreground">"{movie.note}"</p>
        )}
      </div>
    </Link>
  );
