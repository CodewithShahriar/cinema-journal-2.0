...
function WatchlistPage() {
  ...
  const years = useMemo(
    () => Array.from(new Set(movies.map((m) => m.year).filter(Boolean))).sort((a, b) => (b as number) - (a as number)),
    [movies],
  );
  ...
  const list = useMemo(() => {
    ...
    if (year !== "all") out = out.filter((m) => String(m.year) === year);
    ...
    if (sort === "year") out = [...out].sort((a, b) => (b.year ?? 0) - (a.year ?? 0));
    return out;
  ...
  return (
    <AppShell>
      ...
      <div className="mb-3 relative">
        ...
        <Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Search title, genre, note..." className="pl-9" />
      </div>
      ...
      <div className="mb-5 grid grid-cols-2 gap-2">
        ...
        <Select value={sort} onValueChange={setSort}>
          <SelectTrigger><SlidersHorizontal className="h-4 w-4 mr-1" /><SelectValue /></SelectTrigger>
          <SelectContent>
        ...
        <button
          ...
          className={`rounded-md border px-3 text-sm transition-colors ${favOnly ? "border-primary bg-primary/10 text-primary" : "border-input bg-transparent"}`}
        >
      ...
      {list.length === 0 ? (
        <EmptyState icon={Bookmark} title="Your watchlist is empty" hint="Add movies you plan to watch — they'll appear here." />
      ) : (
